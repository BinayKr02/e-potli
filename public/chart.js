  ...
  .then((f_res) => {
    ...
    function D3(svg) {
      const self = {
        ...
        append: (shape) => {
          let created = self.html.appendChild(
          ...
          const layout = {
            ...
            hover: (callBack) => {
              created.addEventListener("mouseover", callBack);
              return layout;
            },
            attr: (Name, value = "") => {
            ...
            append: (shape) => {
              let create = created.appendChild(
                  document.createElementNS("http://www.w3.org/2000/svg", shape)
                ),
                added = {
                  x: (number) => {
                    create.setAttribute("x", number);
                    return added;
                  },
                  y: (number) => {
                    create.setAttribute("y", number);
                    return added;
                  },
                  rx: (number) => {
                    create.setAttribute("rx", number);
                    return added;
                  },
                  attr: (Name, value = "") => {
                    create.setAttribute(Name, value);
                    return added;
                  },
                  width: (number) => {
                    create.setAttribute("width", number + "px");
                    return added;
                  },
                  height: (number) => {
                    create.setAttribute("height", number + "px");
                    return added;
                  },
                  fill: (color) => {
                    create.setAttribute("fill", color);
                    return added;
                  },
                  stroke: (color) => {
                    create.setAttribute("stroke", color);
                    return added;
                  },
                  strokeWidth: (number) => {
                    create.setAttribute("stroke-width", number + "px");
                    return added;
                  },
                  text: (text) => {
    ...
    let price = parseFloat(
      document.getElementById("investment").innerHTML.substr(1)
    );
    console.log(f_res.priceList[9].price);
    console.log(typeof f_res.priceList[9].price);
    let data = {
        "T-9 day": price * f_res.priceList[9].price,
        "T-8 day": price * f_res.priceList[8].price,
        "T-7 day": price * f_res.priceList[7].price,
        "T-6 day": price * f_res.priceList[6].price,
        "T-5 day": price * f_res.priceList[5].price,
        "T-4 day": price * f_res.priceList[4].price,
        "T-3 day": price * f_res.priceList[3].price,
        "T-2 day": price * f_res.priceList[2].price,
        Yesterday: price * f_res.priceList[1].price,
        Today: price * f_res.priceList[0].price,
      },
      w = 600,
      h = 300,
      x_space = 20,
    ...
    for (let circle of document.querySelectorAll("svg circle[hovering]")) {
      ...
      circle.addEventListener("mouseover", (_) => {
        ...
        document
          .querySelector(
            `circle[animetion][unique="${circle.getAttribute("unique")}"]`
          )
          .setAttribute("start", "");
      });
      circle.addEventListener("mouseout", (_) => {
        document
          .querySelector(`svg g[unique="${circle.getAttribute("unique")}"]`)
          .remove();
        document
          .querySelector(
            `circle[animetion][unique="${circle.getAttribute("unique")}"]`
